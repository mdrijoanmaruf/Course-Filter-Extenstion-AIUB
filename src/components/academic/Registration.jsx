...
function schedTypeClass(type) {
  ...
  if (t === 'theory') return 'bg-blue-50 text-blue-700';
  if (t === 'lab') return 'bg-green-50 text-green-700';
...
function CourseCard({ course }) {
  ...
  return (
    <div className={`border rounded-xl overflow-hidden bg-white transition-all hover:shadow-md ${
      isDropped ? 'border-red-200 bg-red-50 hover:border-red-300' : 'border-slate-200 hover:border-blue-200'
    }`}>
      <div className="flex items-start gap-2.5 px-3.5 pt-2.5 pb-2">
        <div className="flex-1 min-w-0">
          <div className={`font-mono text-[11px] font-semibold tracking-wide mb-0.5 ${isDropped ? 'text-slate-400' : 'text-blue-900'}`}>
            <a href={safeHref} style={{ color: 'inherit', textDecoration: 'none' }}>{course.code}</a>
          </div>
          <div className={`text-[13px] font-semibold leading-snug ${isDropped ? 'text-slate-400 line-through' : 'text-slate-900'}`}>
            {course.name}
          </div>
        </div>
        {course.section && (
          <div className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center text-[14px] font-extrabold border ${
            isDropped ? 'bg-slate-100 border-slate-200 text-slate-400' : 'bg-blue-50 border-blue-200 text-blue-700'
          }`}>
            {course.section}
          </div>
        )}
      ...
      {course.schedules.length > 0 && (
        <div className="px-3.5 pb-2 flex flex-col gap-1">
          {course.schedules.map((s, i) => (
            <div key={i} className="flex items-center gap-1.5 text-[11px] text-slate-500 flex-wrap">
              <span className={`text-[10px] font-bold py-0.5 px-1.5 rounded flex-shrink-0 ${schedTypeClass(s.type)}`}>
                {s.type || 'Time'}
              </span>
              <span className="text-slate-500">{s.time}</span>
              {s.room && <span className="ml-auto font-mono text-[10px] font-bold text-slate-400 whitespace-nowrap">{s.room}</span>}
            </div>
          ))}
        </div>
      )}

      <div className="flex items-center justify-between px-3.5 pt-1 pb-2.5 gap-2">
        <span className="font-mono text-[10px] font-semibold text-slate-400">{course.credStr}</span>
        {isDropped && (
          <span className="inline-flex items-center gap-1 bg-red-100 text-red-600 rounded px-2 py-0.5 text-[10px] font-bold">
            ⚠ Dropped {dropDetail}
          </span>
        )}
      </div>
...
function FeeRow({ item }) {
  ...
  const lblClass = {
    normal: isZero ? 'text-slate-200' : 'text-slate-500',
    total: 'font-bold text-slate-900',
    'net-total': 'font-bold text-slate-900',
    deduction: 'text-amber-600',
    prev: 'text-amber-600',
    paid: 'font-semibold text-slate-500',
    balance: 'font-bold text-slate-900',
  }[item.type] || 'text-slate-500';

  const amtClass = {
    normal: isZero ? 'text-slate-200' : 'text-slate-600',
    total: 'font-bold text-blue-600 text-[13px]',
    'net-total': 'font-extrabold text-green-600 text-[13px]',
    deduction: 'text-amber-600',
    prev: 'text-amber-600',
    paid: 'text-slate-500',
    balance: `font-extrabold text-[14px] ${num > 0 ? 'text-red-600' : 'text-green-600'}`,
  }[item.type] || 'text-slate-600';

  return (
    <>
      {needsDivider && <div className="h-px bg-slate-200 my-1" />}
      <div className="flex items-center justify-between px-3.5 py-1.5 gap-2 hover:bg-slate-50">
...
function RegistrationView({ semOptions, printHref, creditItems, courses, fees, onSemChange }) {
  return (
    <div className="text-[13px] text-slate-800" style={{ boxSizing: 'border-box' }}>
      {/* Header */}
      <div className="flex items-center justify-between flex-wrap gap-2.5 mb-4 pb-3 border-b-2 border-slate-100">
        <h2 className="text-[16px] font-bold text-slate-900 tracking-tight m-0">
          Course <span className="text-blue-600">Registration</span>
        </h2>
        <div className="flex items-center gap-2.5 flex-wrap">
          {semOptions.length > 0 && (
            <select
              className="text-[12px] font-semibold text-slate-700 border border-slate-300 rounded-lg px-2.5 py-1.5 bg-slate-50 cursor-pointer outline-none focus:border-blue-300 focus:ring-1 focus:ring-blue-200"
          ...
          {printHref && (
            <a href={printHref} className="text-[11px] font-semibold text-slate-500 border border-slate-300 rounded-md px-3 py-1.5 bg-slate-50 hover:bg-red-50 hover:text-red-600 hover:border-red-300 transition-all no-underline whitespace-nowrap">
              🖨 Print
            </a>
          )}
        </div>
      ...
      {creditItems.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-4">
          {creditItems.map(({ key, val }) => {
            ...
            return (
              <span key={key} className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[11px] font-semibold border ${
                active ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-slate-100 border-slate-200 text-slate-400'
              }`}>
                <span className="font-extrabold text-[13px]">{val}</span>{key}
              </span>
            );
          })}
        </div>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: fees.length ? '1fr 270px' : '1fr' }}>
        ...
        {fees.length > 0 && (
          <div className="border border-slate-200 rounded-xl overflow-hidden bg-white" style={{ position: 'sticky', top: 16, alignSelf: 'start' }}>
            <div className="px-3.5 py-2.5 bg-slate-50 border-b border-slate-200 text-[11px] font-bold uppercase tracking-wider text-slate-500">
              Fee Assessment
            </div>
            <div className="py-1">
